...
def test_backoff_timing(healer):
    ...
    assert len(sleep_times) == 2, f"Expected 2 sleep calls, got {len(sleep_times)}: {sleep_times}"
    assert sleep_times[0] == 1, f"First backoff should be 1s (2^0), got {sleep_times[0]}"
    assert sleep_times[1] == 2, f"Second backoff should be 2s (2^1), got {sleep_times[1]}"
