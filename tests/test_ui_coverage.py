...
def test_load_css_exists():
    ...
    with patch("blast_ocr.ui.web_app.Path.exists", return_value=True):
        with patch("blast_ocr.ui.web_app.Path.read_text", return_value="body { color: black; }"):
...
def test_handle_file_upload_unauthorized_extension():
    ...
    mock_db = MagicMock()
    mock_state = MockSessionState({"session_id": "test-uuid", "output_dir": "test_out", "current_results": None})
    ...
    with patch('streamlit.file_uploader', return_value=[mock_file]):
        with patch('streamlit.button', return_value=True):
            with patch('streamlit.session_state', mock_state):
                ...
                assert "UNAUTHORIZED EXTENSION" in mock_state.current_results["summary"][0]["ERROR"]
...
def test_handle_file_upload_pipeline_failure():
    ...
    mock_pipeline.process_job.return_value = {"status": "failed", "error": "Poppler timeout"}
    ...
    with patch('streamlit.file_uploader', return_value=[mock_file]):
        with patch('streamlit.button', return_value=True):
            with patch('streamlit.session_state', mock_state):
                web_app.handle_file_upload(mock_pipeline, mock_db)
                assert mock_state.current_results["summary"][0]["STATUS"] == "FAILED"
                assert "Poppler timeout" in mock_state.current_results["summary"][0]["ERROR"]
...
def test_main_ui_flow():
    ...
    with patch('streamlit.session_state', mock_state):
        with patch('streamlit.set_page_config'), patch('streamlit.markdown'), patch('streamlit.metric'), \
             ...
             with patch('streamlit.radio', return_value="HANDWRITING ANALYSIS"):
                 with patch('streamlit.button', return_value=True): # Purge logs button is True
    ...
    with patch('streamlit.session_state', MockSessionState({"processing_history": [], "total_scans": 0, "pages_decoded": 0, "session_id": "1", "output_dir": None, "current_results": None})):
        with patch('streamlit.set_page_config'), patch('streamlit.markdown'), patch('streamlit.metric'), \
             patch('streamlit.columns', return_value=(MagicMock(), MagicMock())), \
             patch('streamlit.tabs', return_value=mock_tabs.tabs):
