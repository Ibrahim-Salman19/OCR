...
def test_save_result_invalid_fk_job_id(fresh_db):
    ...
    try:
        fresh_db.save_result(
            ...
            page_number=1, text="orphaned", confidence=0.9, processing_time=1.0
        )
...
def test_create_job_sql_injection_in_filename(fresh_db):
    ...
    assert job.filename == malicious, "Filename was modified (should be stored verbatim)"
...
def test_two_database_instances_same_file():
    ...
    try:
        ...
        if errors:
            pytest.fail(f"Two OCRDatabase instances on same file caused errors: {errors[:3]}")
    finally:
        try:
            db1.close(); db2.close()
            import time
...
def test_database_uses_begin_immediate():
    ...
    uses_immediate = (
        "IMMEDIATE" in source or
        "isolation_level" in source or
        "begin_nested" in source
    )
...
def test_database_exceptions_trigger_rollback(fresh_db):
    ...
    with patch.object(fresh_db.session, "commit", side_effect=Exception("Mock Commit Error")):
