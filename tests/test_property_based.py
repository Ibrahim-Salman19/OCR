...
def test_sanitize_always_returns_string(text):
    ...
    assert isinstance(result, str), f"sanitize_for_xml returned {type(result)} for input {repr(text)}"
...
@given(
    key=st.text(alphabet=st.characters(whitelist_categories=('Ll', 'Lu', 'Nd')),
...
def test_process_job_never_returns_none(path):
    ...
    assert isinstance(result, dict), f"process_job returned {type(result)} for path: {path!r}"
