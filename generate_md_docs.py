...
def generate_md():
    ...
    out.append(
        f"> Generated on: {datetime.now().strftime('%Y-%m-%d')} | Author: devhms | Branch: main\n"
    )
    ...
    def add_file(filepath, description=""):
        if os.path.exists(filepath):
            ...
            lang = (
                ext
                if ext
                in ["py", "css", "html", "js", "json", "toml", "yaml", "md", "sh"]
                else "text"
            )
            if ext == "":
                lang = "text"
    ...
    # Section 5
    add_section(
        "5. Caching & Storage", "AI Agent", "Performance optimization and persistence."
    )
    ...
    add_file(
        "blast_ocr/ui/styles.css", "Custom CSS Injection for Premium Glassmorphism Look"
    )
    ...
    add_file(
        "blast_ocr/core/exceptions.py", "Custom Exceptions and Error Classifications"
    )
    ...
    add_file(
        "blast_ocr/logging_config.py", "Structured JSON and Desktop Loging Strategies"
    )
