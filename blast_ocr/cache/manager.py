...
try:
    ...
    USE_ORJSON = False
    logger.debug(
        "orjson not available, using stdlib json (install orjson for 2-5x faster cache)"
    )
...
class OCRCache:
    ...
    def __init__(self, cache_dir=None):
        ...
        self._lock = (
            threading.Lock()
        )  # FIX(phase3): Prevent race conditions on file writes
        ...
        with self._lock:  # FIX(phase3): Thread-safe write
            try:
                ...
                fd, temp_path = tempfile.mkstemp(
                    dir=str(self.cache_dir), prefix=".tmp_"
                )
