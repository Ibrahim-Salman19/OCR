...
def add_toc(doc):
    ...
    p = doc.add_paragraph()
    p.add_run(
        "Note: If the Table of Contents is empty, right-click here and select 'Update Field' to generate it."
    ).italic = True
...
def add_file_content(
    doc, title, file_path, desc="Source file content extracted verbatim."
):
...
def generate_doc():
    ...
    add_infobox(
        doc, "USER / BUSINESS", "High-level overview of the project goal and outcomes."
    )
    add_paragraph(
        doc,
    ...
    add_paragraph(
        doc,
        "To quickly get started with the B.L.A.S.T. OCR engine, use the following commands from your terminal:",
    )
    add_code_block(
        doc,
        "# Run CLI OCR Pipeline\npython run.py --source input.pdf --output out_dir\n\n# Launch Web Dashboard\nstreamlit run run_gui.py",
    )
    ...
    add_infobox(
        doc, "ALL", "The foundational principles guiding B.L.A.S.T. development."
    )
    add_paragraph(
        doc,
        "The project strictly adheres to the A.N.T. (Architect, Navigate, Tool) philosophy, providing a robust modular architecture to manage dependencies and system state. The Source of Truth for the design intent is maintained in gemini.md.",
    )
    add_file_content(
        doc, "gemini.md", "gemini.md", "Project Master Plan & Source of Truth"
    )
    add_file_content(
        doc, "ARCHITECTURE.md", "ARCHITECTURE.md", "A.N.T. Design Philosophy"
    )
    ...
    add_file_content(
        doc,
        "extraction_flow.md",
        "architecture/extraction_flow.md",
    ...
    add_file_content(
        doc, ".env.example", ".env.example", "Environment Variables Template"
    )
    add_file_content(
        doc, "dll_check.py", "dll_check.py", "Windows Environment Integrity Check"
    )
    add_file_content(
        doc,
    ...
    add_infobox(
        doc,
        "DEVELOPER",
        "Advanced error recovery strategies and custom exception taxonomies.",
    )
    add_file_content(
        doc,
        "blast_ocr/core/healing.py",
        "blast_ocr/core/healing.py",
        "Retry loops and back-off architectures",
    )
    add_file_content(
        doc,
        "blast_ocr/core/exceptions.py",
        "blast_ocr/core/exceptions.py",
    ...
    add_infobox(
        doc, "DEVELOPER", "Multi-threading utilities tuned for optimal GPU/CPU load."
    )
    add_file_content(
        doc,
    ...
    add_file_content(
        doc, "blast_ocr/config.py", "blast_ocr/config.py", "Pydantic Settings Models"
    )
    add_file_content(
        doc,
        "blast_ocr/logging_config.py",
        "blast_ocr/logging_config.py",
    ...
    add_infobox(
        doc, "DEVELOPER", "Project maintenance utilities to track system erosion."
    )
    add_file_content(doc, "AUDIT.md", "AUDIT.md", "Performance and Memory Leak Audit")
    add_file_content(
        doc, "maintain.py", "maintain.py", "Cleanup and health check automations"
    )
    add_file_content(
        doc,
        "inventory_gen.py",
    ...
    add_file_content(
        doc, "ENHANCEMENTS.md", "ENHANCEMENTS.md", "Visual and Logic upgrades"
    )
    ...
    add_file_content(
        doc, "DEVTOOLS_GUIDE.md", "DEVTOOLS_GUIDE.md", "Frontend Debugging Patterns"
    )

    add_heading(doc, "Section 18 — Skill Knowledge Base", 1)
    add_infobox(
        doc, "DEVELOPER", "Distilled logic and patterns for maintaining the engine."
    )
    ...
    add_paragraph(
        doc,
        "System dependencies are explicitly mapped in Section 5 (Environment). Ensure appropriate Docker/Poppler binaries are provisioned downstream.",
    )

    add_heading(doc, "Section 20 — Benchmarking", 1)
    add_infobox(doc, "DEVELOPER", "Metrics verification scripts.")
    add_file_content(
        doc, "benchmark.py", "benchmark.py", "Measures execution speeds and limits."
    )
