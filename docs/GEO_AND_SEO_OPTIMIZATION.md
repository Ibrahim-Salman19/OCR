...
- **CI, confirmed live**: after the fixes above, `gh run watch` was used to actually observe two full CI runs on `github.com/Ibrahim-Salman19/OCR/actions` rather than trust the local reproduction alone. The first surfaced two more real, CI-environment-only bugs the collection crash had been hiding — `blast_ocr/core/engines/tesseract_engine.py` importing `pytesseract` unconditionally where a mockable/optional reference existed (broke in CI, which doesn't install pytesseract; masked locally because it happened to be installed there), and a test with its return-value unpacking backwards (`tests/test_ui_deep_coverage.py::test_resource_snapshot`). Both fixed and pushed. A third failure (a 3-point OLS memory-slope regression, `test_extreme_stress_runner_suites_end_to_end`) passed 3/3 locally with wide margins and was left alone rather than guess-fixed; a second live CI run confirmed it passing too, on real GitHub Actions infrastructure: **all 6 jobs green, 840 passed / 3 skipped / 0 failed** (run `34042926985`).
