...
* **Known Strengths**: High throughput (no GPU throughput number has been measured for this project -- see docs/BENCHMARKS_2026.md), zero-disk array passing, structured `page_model` emission.
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **`core/engines/`** | Validates array finiteness, shapes, non-empty streams, positive dimensions. | Typed `OCREngineInitializationError`; Tesseract missing binary falls back to RapidOCR; Ensemble votes on confidence. | Memory tensors sliced into dynamic mini-batches; lazy engine initialization. | Thread-safe singleton session cache. | High throughput (~15.3s/page measured on CPU, 7.7x faster than the prior EasyOCR baseline; no GPU number measured); multi-engine consensus; zero synthetic OCR mocks. | Empty page OCR returns 0.0 confidence and empty text; downstream callers must handle blank pages. |
